...
class Product {
    public static function initSQLConnection() {
        ...
        if ($resource === false) {
            throw new Exception("Could not connect to MySQL server: " . mysqli_error($resource));
        }
        if (!mysql_select_db('test', $resource)) {
            throw new Exception("Could not use database 'test': " . mysqli_error($resource));
        }
    }
    /*
    ...
    public static function delete($aId) {
        ...
        $result = mysqli_query("DELETE FROM `products` WHERE `id`=" . $lId);
        return $result;
    }
    /*
